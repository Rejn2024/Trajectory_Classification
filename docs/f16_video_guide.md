...
4. Enable the BVR Sim ACMI/Tacview logger using the option documented by the pinned checkout. Open the resulting `.acmi` in Tacview for a combat-replay visualization. For the ACMI files generated by `02_generate_jsbsim_skill_dataset.ipynb`, the [Tacview visualisation and video-recording guide](tacview_video_guide.md) covers verification, camera setup, OBS capture, post-processing, and troubleshooting.
